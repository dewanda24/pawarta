...
export function ActivityLogsClient({ initialLogs }: ActivityLogsClientProps) {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      if (modulFilter !== 'ALL' && log.modul !== modulFilter) return false;

      if (searchQuery.trim()) {
        const q = searchQuery.toLowerCase();
        const matchAktor = log.aktorNama?.toLowerCase().includes(q);
        const matchUsername = log.aktorUsername?.toLowerCase().includes(q);
        const matchAksi = log.aksi?.toLowerCase().includes(q);
        const matchModul = log.modul?.toLowerCase().includes(q);
        const matchDetail = log.detailAktivitas?.toLowerCase().includes(q);
        const matchIp = log.ipAddress?.toLowerCase().includes(q);
        if (!matchAktor && !matchUsername && !matchAksi && !matchModul && !matchDetail && !matchIp) return false;
      }
      return true;
    });
  }, [logs, modulFilter, searchQuery]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Jejak audit komprehensif seluruh transaksi pencatatan, perubahan data, approval, dan penerbitan naskah
            </p>
          </div>
        </div>
        <Badge variant="outline" className="px-3 py-1 text-xs bg-slate-50 border-slate-200 text-slate-700 font-medium">
          Total {logs.length} Transaksi Terakhir
        </Badge>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
          <div className="sm:col-span-8 relative">
            ...
            <Input
              ...
              placeholder="Cari aktor, aksi, modul, detail aktivitas, atau alamat IP..."
          ...
          <div className="sm:col-span-4">
            <Select value={modulFilter} onValueChange={setModulFilter}>
              ...
              <SelectContent>
                <SelectItem value="ALL">Semua Modul ({logs.length})</SelectItem>
                {modulOptions.map((m) => (
              ...
              {paginatedLogs.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-gray-400">
                    Tidak ada riwayat aktivitas sistem yang sesuai filter.
                  </td>
                </tr>
              ) : (
                paginatedLogs.map((log) => {
                  ...
                  if (aksiUpper.includes('CREATE') || aksiUpper.includes('TAMBAH')) {
                    badgeStyle = 'bg-emerald-50 text-emerald-700 border-emerald-200';
                  } else if (aksiUpper.includes('UPDATE') || aksiUpper.includes('UBAH')) {
                    badgeStyle = 'bg-blue-50 text-blue-700 border-blue-200';
                  } else if (aksiUpper.includes('DELETE') || aksiUpper.includes('HAPUS')) {
                    badgeStyle = 'bg-red-50 text-red-700 border-red-200';
                  } else if (aksiUpper.includes('APPROVE') || aksiUpper.includes('SIGN')) {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="font-semibold text-gray-900">{log.aktorNama || 'Sistem'}</div>
                        <div className="text-[11px] text-gray-500 font-mono">
                          {log.aktorUsername ? `@${log.aktorUsername}` : 'System'}
                        </div>
                      </td>

                      {/* Aksi */}
                      <td className="px-5 py-4">
                        <Badge variant="outline" className={`font-mono text-[11px] ${badgeStyle}`}>
                          {log.aksi}
                        </Badge>
                      </td>
                      ...
                      {/* Detail */}
                      <td className="px-5 py-4 text-gray-800 max-w-sm">
                        <p className="line-clamp-2 leading-relaxed">{log.detailAktivitas}</p>
                      </td>
