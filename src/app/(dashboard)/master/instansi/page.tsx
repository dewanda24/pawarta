...
import { InstansiForm, InstansiFormValues } from './form';
import { DeleteConfirmDialog } from '@/components/shared/delete-confirm-dialog';
import { getInstansiList, deleteInstansi } from '@/features/master-data/actions/instansi';
...
export default function InstansiPage() {
  ...
  const [selectedData, setSelectedData] = useState<InstansiFormValues & { id?: string } | undefined>(undefined);
  ...
  const fetchData = async () => {
    try {
      const response = await getInstansiList({
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await deleteInstansi(deletingId);
      if (res.success) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Master Instansi</h1>
          <p className="text-sm text-gray-500">Kelola data instansi tujuan atau asal surat.</p>
        </div>
