...
import { JabatanForm, JabatanFormValues } from './form';
import { DeleteConfirmDialog } from '@/components/shared/delete-confirm-dialog';
import { getJabatanList, deleteJabatan } from '@/features/master-data/actions/jabatan';
...
export default function JabatanPage() {
  ...
  const [selectedData, setSelectedData] = useState<JabatanFormValues & { id?: string } | undefined>(undefined);
  ...
  const fetchData = async () => {
    try {
      const response = await getJabatanList({
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await deleteJabatan(deletingId);
      if (res.success) {
