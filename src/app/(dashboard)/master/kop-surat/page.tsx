      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
        <div>
          ...
          <p className='text-sm text-gray-500'>
            Kelola kepala naskah dinas (KOP Surat) resmi sekolah yang digunakan pada seluruh cetakan surat dinas dan kesiswaan.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className='text-center py-16 border-2 border-dashed border-gray-200 rounded-2xl bg-white p-8'>
          ...
          <p className='text-xs text-gray-500 max-w-md mx-auto mt-1 mb-4'>
            Sistem saat ini menggunakan KOP default dari profil sekolah. Anda dapat membuat desain KOP surat resmi dinas baru sekarang.
          </p>
          <Button
            onClick={() => {
              setSelectedData(null);
          ...
          {data.map((item) => (
            <div
              ...
              className={`rounded-2xl border bg-white p-6 shadow-xs flex flex-col justify-between transition-all ${
                item.isDefault ? 'border-blue-500 ring-2 ring-blue-500/20 shadow-md' : 'border-gray-200 hover:border-gray-300'
              }`}
            >
              <div>
                {/* Header Card */}
                ...
                <div className='bg-gray-50/70 p-4 sm:p-5 rounded-xl border border-gray-200 mb-4'>
                  <div className='flex items-center justify-between gap-3'>
                    ...
                    <div className='flex-1 text-center px-1'>
                      <h4 className='text-[10px] font-bold uppercase tracking-wider text-gray-700 font-sans'>
                        {item.instansiUtama || 'INSTANSI INDUK / DINAS PENDIDIKAN'}
                      </h4>
                      <h2 className='text-xs sm:text-sm font-extrabold uppercase tracking-tight text-gray-950 mt-0.5 font-sans'>
                        {item.namaSekolah || 'NAMA SEKOLAH'}
                      </h2>
                    ...
                    <div className='w-14 sm:w-16 shrink-0 hidden sm:block' />
                  </div>

                  <div
                    className={`mt-2.5 ${
                      item.tipeGaris === 'double_thick'
                        ? 'border-b-4 border-double border-gray-950'
                        : item.tipeGaris === 'single_thick'
