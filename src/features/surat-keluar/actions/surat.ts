...
export async function getSuratKeluarList(params?: { search?: string; limit?: number; offset?: number }) {
  try {
...
export async function createSuratKeluar(data: InsertOutgoingLetter) {
  try {
    ...
    const [inserted] = await db.insert(outgoingLetters).values({
      ...data,
      status: 'DRAFT',
    }).returning();
    ...
    return { success: false, error: error.message || 'Gagal membuat surat keluar' };
  }
}
...
export async function updateSuratKeluar(id: string, data: Partial<InsertOutgoingLetter>) {
  try {
    ...
    return { success: false, error: error.message || 'Gagal mengubah surat keluar' };
  }
}
...
export async function deleteSuratKeluar(id: string) {
  try {
    ...
    return { success: false, error: error.message || 'Gagal menghapus surat keluar' };
  }
}
