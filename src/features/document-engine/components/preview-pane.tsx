...
export function PreviewPane({ previewMode, onClosePreview, validationResult }: PreviewPaneProps) {
  return (
    <div
      className={`flex-1 bg-gray-100 overflow-y-auto ${!previewMode ? 'hidden md:flex md:w-1/2' : 'w-full'} flex-col items-center py-8 relative`}
    >
            ...
            <>
              {validationResult.missing.length > 0 && (
                <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded text-red-600 text-sm">
                  <span className="font-bold">Error:</span> Terdapat placeholder yang tidak
                  terdaftar: {validationResult.missing.join(', ')}
                </div>
              )}
