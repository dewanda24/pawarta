...
export const outgoingLetters = pgTable('outgoing_letters', {
  ...
  jenisSuratId: uuid('jenis_surat_id').references(() => masterJenisSurat.id, { onDelete: 'restrict' }),
  klasifikasiId: uuid('klasifikasi_id').references(() => masterKlasifikasiSurat.id, { onDelete: 'restrict' }),
  prioritasId: uuid('prioritas_id').references(() => masterPrioritas.id, { onDelete: 'set null' }),
  sifatSuratId: uuid('sifat_surat_id').references(() => masterSifatSurat.id, { onDelete: 'set null' }),
  perihal: text('perihal').notNull(),
  tujuanSurat: varchar('tujuan_surat', { length: 255 }).notNull(), // Personal tujuan
  instansiTujuanId: uuid('instansi_tujuan_id').references(() => masterInstansi.id, { onDelete: 'set null' }), // Organisasi tujuan
  pembuatId: uuid('pembuat_id').references(() => users.id, { onDelete: 'restrict' }).notNull(), // Konseptor
  unitKerjaId: uuid('unit_kerja_id').references(() => masterUnitKerja.id, { onDelete: 'restrict' }).notNull(),
  penandatanganId: uuid('penandatangan_id').references(() => masterPegawai.id, { onDelete: 'restrict' }),
...
export const letterSignatures = pgTable('letter_signatures', {
  suratId: uuid('surat_id')
    .notNull()
    .references(() => outgoingLetters.id, { onDelete: 'cascade' }),
  penandatanganId: uuid('penandatangan_id').references(() => masterPegawai.id, { onDelete: 'restrict' }).notNull(),
